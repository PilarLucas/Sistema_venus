  ...
  if (empty($_POST['producto']) || empty($_POST['precio'])) {
    ...
    if(!$_FILES['imagen']['tmp_name']){
      $query_update = mysqli_query($conexion, "UPDATE producto SET descripcion = '$producto', proveedor= $proveedor,precio= $precio WHERE codproducto = $codproducto");
        ...
        $query_update = mysqli_query($conexion, "UPDATE producto SET descripcion = '$producto', proveedor= $proveedor,precio= $precio, imagen='$imagen'  WHERE codproducto = $codproducto");
      }
...
if (empty($_REQUEST['id'])) {
  ...
  $query_producto = mysqli_query($conexion, "SELECT p.codproducto, p.descripcion, p.precio, pr.codproveedor, pr.proveedor, p.imagen FROM producto p INNER JOIN proveedor pr ON p.proveedor = pr.codproveedor WHERE p.codproducto = $id_producto");
        ...
        <div class="card-body">
          <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <div><img src="data:img/jpg;base64,<?php echo base64_encode($data_producto['imagen']);?>"></div>
