 <?php include_once "includes/header.php";
  ...
  if (!empty($_POST)) {
    ...
    if (empty($_POST['proveedor']) || empty($_POST['producto']) || empty($_POST['precio']) || $_POST['precio'] <  0 || empty($_POST['cantidad'] || $_POST['cantidad'] <  0)) {
      ...
      $imagen = addslashes(file_get_contents($_FILES['ima']['tmp_name']));

      $query_insert = mysqli_query($conexion, "INSERT INTO producto(proveedor,descripcion,precio,existencia,usuario_id,imagen) values ('$proveedor', '$producto', '$precio', '$cantidad','$usuario_id', '$imagen')");
      if ($query_insert) {
